...
function createElemWithText(element = "p", textContent = "", className){
    const myElem = document.createElement(element);
...
if(!usersData) return undefined; //if there is no parameter return undefined
let optArray = [];
        for(let i = 0; i < usersData.length; i++){ //iterate through the users data
            const newOptElement = document.createElement("option");//creating an option element for each user
            newOptElement.value = usersData[i].id;
            newOptElement.textContent = usersData[i].name; 
            optArray.push(newOptElement); //adding each option element to the array to return
        }
    return optArray;
};
...
At this point in your code, the section will not exist. You can create one to test if desired.
Toggles the class 'hide' on the section element Return the section element
 */
function toggleCommentSection(postId){
    if(!postId) return; //if no parameter is passed
...
will pass for addButtonListeners until toggleComments exists. I recommend waiting on the logic inside the toggleComments function until we get there.
*/
function addButtonListeners(){
    ...
    if(btns){
        for(let i = 0; i<btns.length; i++){
            const postId = btns[i].dataset.postId;
...
function removeButtonListeners(){
    ...
    if(btns){
        for(let i = 0; i<btns.length; i++){
            const postId = btns[i].dataset.postId;
            btns[i].removeEventListener("click", function(e){
            toggleComments(e, postId)}, false );
        }//for
    }//if
    return btns;
